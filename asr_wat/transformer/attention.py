...
class AdpHeadedAttention(nn.Module):
    ...
    def __init__(self, n_head, n_feat, dropout_rate, threshold):
        """Construct an MultiHeadedAttention object."""
        super(AdpHeadedAttention, self).__init__()
        assert n_feat % n_head == 0
        self.d_k = n_feat // n_head
        self.h = n_head
        self.linear_q = nn.Linear(256, n_feat)
        self.linear_k = nn.Linear(256, n_feat)
        self.linear_v = nn.Linear(256, n_feat)
        self.linear_out = nn.Linear(n_feat, 256)
        self.attn = None        
        self.dropout = nn.Dropout(p=dropout_rate)       
        self.threshold = threshold
        self.scale_a = torch.nn.Parameter(torch.FloatTensor(torch.ones(n_feat)))
        self.scale_base = torch.ones(n_feat)
        self.scale_max = None
